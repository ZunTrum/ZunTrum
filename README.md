...
page2:Toggle("Rare and Ultra Rare Fruit Checking", false, function(rnu)
    ...
    while getgenv().checkrnu do
        wait() -- Prevents the loop from running too fast
        pcall(function()
            local UltraRares = {"Dark Fruit", "Quake Fruit", "Vampire Fruit", "Phoenix Fruit"}
            local Rares = {"Alice Fruit", "Gas Fruit", "Plasma Fruit", "Snow Fruit", "Magma Fruit", 
                           "Rumble Fruit", "Sand Fruit", "Light Fruit", "Flare Fruit", "Smoke Fruit", 
                           "Chilly Fruit", "Hollow Fruit", "Candy Fruit", "Venom Fruit", "Ope Fruit", 
                           "Gravity Fruit"}
...
page2:Toggle("Aura Fruit Checking", false, function(Aur)
    getgenv().checkaur = Aur
    while getgenv().checkaur do wait()
        pcall(function()
            for _,c in pairs(game.Players:GetChildren()) do
                for i,v in pairs(c.Backpack:GetChildren()) do
                if v:FindFirstChild("DF_Script") then
                    for _,k in pairs(v:GetChildren()) do
                        if k:FindFirstChild("AuraAttachment") then
                                                                game.StarterGui:SetCore("SendNotification", {
                                            Title = "AURA Notifier", 
                                            Text = c.Name .. ": " .. v.Name .." AURA!!", 
                                            Duration = 1
                                        })
                            end
                        end
...
page2:Toggle("Auto Open Box Uncommon and Common", false, function(fruitbox)
getgenv().checkfruitbox = fruitbox
while getgenv().checkfruitbox do wait()
    for _,v in pairs(game.Players.LocalPlayer.Backpack:GetChildren()) do
        if v:IsA("Tool") and string.find(v.Name,"Uncommon") or string.find(v.Name,"Common") then
...
page5:Line()
    ...
    PlayerName = {}
        ...
        page5:Toggle("Toxic Text", false, function(x)
            getgenv().toxictext = x
            while getgenv().toxictext do wait()
                pcall(function()
                if game:GetService("Workspace"):FindFirstChild(SelectedKillPlayer) then
                if game:GetService("Workspace"):FindFirstChild(SelectedKillPlayer).Humanoid.Health <= 0 and not game:GetService("Workspace"):FindFirstChild(SelectedKillPlayer).Humanoid.PlatformStand then
                            local args = {
    [1] = "/w ".. SelectedKillPlayer .. " " ..totaltext[math.random(1,#totaltext)],
    [2] = "All"
}
game:GetService("ReplicatedStorage"):WaitForChild("DefaultChatSystemChatEvents"):WaitForChild("SayMessageRequest"):FireServer(unpack(args))

game:GetService("Workspace"):FindFirstChild(SelectedKillPlayer).Humanoid.PlatformStand = true
     end
    end
                end)
            end
        end)
        page5:Button("Stats Check",function()
local UserData = workspace.UserData["User_"..game.Players:FindFirstChild(SelectedKillPlayer).UserId]
local totalstorefruit = {}
print("\n Username: "..SelectedKillPlayer.." | Total Level: "..UserData.TotalLevel.Value.."\
Melee: ".. UserData.Data.MeleeLevel.Value.." | Sword: ".. UserData.Data.SwordLevel.Value.." | Sniper: ".. UserData.Data.SniperLevel.Value.." | Defense: ".. UserData.Data.DefenseLevel.Value.." | Expertise: ".. UserData.Data.ExpertiseLevel.Value.. " | Haki: ".. UserData.Data.HakiLevel.Value.."\
Beli: ".. UserData.Data.Cash.Value.." | Gems: ".. UserData.Data.Gems.Value.." | Bounty: ".. UserData.Data.Bounty.Value.." | Kill: ".. UserData.Data.Kills.Value.."")
for i,v in pairs(UserData.Data:GetChildren()) do
    if string.find(v.Name,"StoredDF") and v:IsA("StringValue") and v.Value ~= "None" and not string.find(v.Name,"ID") then
        table.insert(totalstorefruit,"[ ".. v.Value.. " ]")
    end
end
if totalstorefruit[1] then
print(" [ Fruit Store ] \n"..table.concat(totalstorefruit,"\n"))
end
        end)
                            distance = 0
                                ...
                                if v.Name == SelectedKillPlayer then
                                    spawn(function()
                                        pcall(function()
                                            ...
                                            if v.Humanoid.Health <= 0 then
                                            game:GetService("Players").LocalPlayer.Character.Powers.String.RemoteEvent:FireServer(x,"StringPower7")
                                            end
                                        end)
                                    end)
                                end
...
if game.Players.LocalPlayer.Character.Powers.Quake:FindFirstChild("CutEffects") then
game.Players.LocalPlayer.Character.Powers.Quake.CutEffects:Destroy()
end
...
local vppt = x.VTCebvc
    ...
    for i,v in pairs(game:GetService("Workspace"):GetChildren()) do
        if v.Name == Choose2 then
            game:GetService("Players").LocalPlayer.Character.Powers.Quake.RemoteEvent:FireServer(vppt,"QuakePower4","StartCharging","Left")
            ...
            for _, v in pairs(game:GetService("Workspace"):GetChildren()) do
                if v.Name == Choose2 then
                    local argsStart = {
                        ...
                        [2] = "MagmaPower1",
                        [3] = "StartCharging",
                        [4] = CFrame.new(-123.2897, 213, -871.106, 0.99922, -0.00907, 0.03837, 0, 0.97317, 0.23010, -0.03943, -0.22992, 0.97241),
                        [5] = workspace:WaitForChild("IslandTown"):WaitForChild("Beach"):WaitForChild("Beach"),
                        [7] = "Right"
                    }
                    game:GetService("Players").LocalPlayer.Character.Powers.Magma.RemoteEvent:FireServer(unpack(argsStart))
                    wait()
                    ...
                    game:GetService("Players").LocalPlayer.Character.Powers.Magma.RemoteEvent:FireServer(unpack(argsStop))
                    wait()
                    for _, d in pairs(game.Workspace:GetChildren()) do
                        if d.Name ~= game.Players.LocalPlayer.Name and d.Name == SelectedKillPlayer then
                            local humanoidRootPart = d:FindFirstChild("HumanoidRootPart")
                            if humanoidRootPart then
                                pcall(function()
                                    for _, v in pairs(game.Players.LocalPlayer.Character.Powers.Magma.Shots:GetChildren()) do
                                        if v:IsA("Part") then
                                            v.CFrame = humanoidRootPart.CFrame
                                        end
                                    end
...
--------
page5:Toggle("Bomb Camp (Skill 3)", false, function(Maithi)
    getgenv().yeuemmithi = Maithi
    
    while getgenv().yeuemmithi do
        wait()
        
        pcall(function()
            for _, child in pairs(game:GetService("Workspace"):GetChildren()) do
                if child.Name == Choose2 then
                for i = 1,5 do
                spawn(function()
                    local startArgs = {
                        [1] = getsenv(game.Players.LocalPlayer.Character.Powers.Bomb).VTCcpkghc,
                        [2] = "BombPower3",
                        [3] = "StartCharging",
                        [4] = CFrame.new(-123.2897, 213, -871.106, 0.99922, -0.00907, 0.03837, 0, 0.97317, 0.23010, -0.03943, -0.22992, 0.97241),
                        [5] = workspace:WaitForChild("IslandTown"):WaitForChild("Beach"):WaitForChild("Beach"),
                    ...
                    -- Fire StartCharging
                    game:GetService("Players").LocalPlayer.Character.Powers.Bomb.RemoteEvent:FireServer(unpack(startArgs))
                    wait()
                    ...
                    local stopArgs = {
                        ...
                        [2] = "BombPower3",
                        [3] = "StopCharging",
                        [4] = child.HumanoidRootPart.CFrame,
                        [5] = workspace:WaitForChild("IslandTown"):WaitForChild("Beach"):WaitForChild("Beach"),
                        [6] = 100
                    }
                    game:GetService("Players").LocalPlayer.Character.Powers.Bomb.RemoteEvent:FireServer(unpack(stopArgs))
                    wait()
                end)
                wait(0.1)
                end
                    for _, d in pairs(game.Workspace:GetChildren()) do
                        if d.Name == SelectedKillPlayer and d.Name ~= game.Players.LocalPlayer.Name then
                            local humanoidRootPart = d:FindFirstChild("HumanoidRootPart")
                            if humanoidRootPart then
                                for _, v in pairs(game.Players.LocalPlayer.Character.Powers.Bomb.Explosions:GetChildren()) do
                                    if v:IsA("Part") then
                                        v.CFrame = humanoidRootPart.CFrame
                                    end
                                end
